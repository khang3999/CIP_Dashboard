...
class FileService:
    ...
    def create_data_by_excel(self, file: UploadFile, type: str) -> str:
        try:
            ...
            # print(dfs.keys())
            stores = self.supabase.table("cip_stores").select("*").execute().data
            if type == "customers":
                # Chạy qua hàm xử lí cho lounge usage customers cho 2 sheet
                for sheet_name, df in dfs.items():
                    try:
                        # Gọi hàm xử lí dữ liệu ở đây
                        self.process_customer_file(df, sheet_name, stores)
                ...
                for sheet_name, df in dfs.items():
                    try:
                        # Gọi hàm xử lí dữ liệu ở đây
                        self.process_flights_file(df, sheet_name)
                    except Exception as e:
                        print(f"Error processing sheet {sheet_name}: {str(e)}")
                        continue
    ...
    def process_customer_file(self, df: pd.DataFrame, sheet_name: str, stores: list):
        try:
            ...
            if records:
                self.supabase.table("cip_customer_statistics").upsert(records).execute()
    ...
    def process_flights_file(self, df: pd.DataFrame, sheet_name: str):
        try:
            df["timeslot_id"] = df["timeslot"].apply(
                lambda x: 1 if x == 0 else (2 if x == 8 else 3)
            )
            ...
            # Thêm cột region_id (giá trị cố định)
            df["region_id"] = 1 if sheet_name == "QT" else 2
            df["region_id"].fillna(0).astype(int)
            ...
            records = df.to_dict(orient="records")
            if records:
                self.supabase.table("cip_flights").upsert(records).execute()
                print(
                    f"Inserted {len(records)} records flights from sheet {sheet_name}"
                )
            else:
